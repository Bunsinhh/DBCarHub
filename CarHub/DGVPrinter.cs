...
namespace DGVPrinterHelper //AllocationRequest
{
    ...
    public class DGVPrinter
    {
        ...
        /// </summary>
        /// 
        // Add this property to the DGVPrinter class
        public Image HeaderImage { get; set; }

        // Modify the DrawPage method (or equivalent method) in the DGVPrinter class
        private void DrawPage(Graphics g)
        {
            // Print the header image if available
            if (HeaderImage != null)
            {
                // Define the position and size for the image
                PointF imagePosition = new PointF(50, 50); // Adjust as necessary
                SizeF imageSize = new SizeF(HeaderImage.Width, HeaderImage.Height);

                // Draw the image on the graphics object
                g.DrawImage(HeaderImage, new RectangleF(imagePosition, imageSize));
            }

            // Continue with the existing page drawing logic
            // For example:
            // DrawTitle(g);
            // DrawSubTitle(g);
            // DrawFooter(g);
